...
- **🖼️ 圖片與 HTML 檢視** — 圖片支援縮放/平移，HTML 檔以隔離的 sandbox iframe 直接 render。
...
- 問答的對話歷史尚未做修剪，連續多輪長對話可能超出模型 context。
- wikilink 的前處理不會避開 code block，程式碼範例裡的 `[[...]]` 可能被誤轉為連結。
- `![[筆記]]` transclusion 目前只 render 成連結，不會內嵌該筆記內容。
- 預設 vault 路徑寫死於原始碼，需從設定頁更換（首次使用體驗待改善）。
