...
- [ ] 手動「重建索引」按鈕（目前索引失敗只能等 vault 變動才重試）
- [x] 鍵盤快捷鍵（Cmd+K 聚焦搜尋、Cmd+, 開設定、Cmd+F 文內搜尋）
- [ ] 中欄上方的麵包屑列改成多分頁 tabs（可同時開多篇筆記、分頁間切換與關閉）
- [ ] 設定頁擴充：可選 chat / embed 模型、搜尋 top-k
- [ ] Agent 新增 `append_note` 工具（追加比覆寫安全）
- [ ] 欄位拖到底可自動收合（collapsible panels）
- [ ] 問答對話歷史修剪（避免長對話超出模型 context；agent 已有，chat 還沒）
