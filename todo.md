...
- [x] wikilink 前處理會誤轉 code block 內的 `[[...]]`
- [x] `![[筆記]]` transclusion 內嵌（單層 + `#段落`錨點；需實機驗證 render 樣式）
- [x] 圖片「相對於筆記資料夾」的路徑不支援（僅支援 vault 絕對路徑與純檔名）
- [x] 預設 vault 路徑寫死於 `src/main/config.ts`，首次使用體驗待改善（理想為首次啟動引導選擇）
- [~] 自動化測試：已建 Vitest + wikilink/vaultUrl 測試（26 例）；搜尋相關度門檻、agent JSON 協議 parse 待補

## 收尾 / 正式化

- [x] 替換預設 app icon（已換成 1024×1024 Lattic logo，`build/icon.png` + `resources/icon.png`）
- [x] README 截圖：放 `docs/screenshot.jpg` 並打開引用
